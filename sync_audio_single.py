...
def main():
    if len(sys.argv) < 2:
        print("Uso: python sync_audio_single.py <id_del_paragrafo>")
        print("Esempio: python sync_audio_single.py priest_006_3036202d_1783474141319452")
        sys.exit(1)
        
    target_id = sys.argv[1]
    
    print("Caricamento di data.json...")
    ...
    if not target_section:
        print(f"Errore: Nessuna sezione trovata con l'ID '{target_id}' in data.json.")
        sys.exit(1)
        
    if "audio" not in target_section or "text" not in target_section or "it" not in target_section["text"]:
        print(f"Errore: La sezione '{target_id}' non ha audio o testo in italiano.")
        sys.exit(1)
        
    audio_file = target_section["audio"]
    if not os.path.exists(audio_file):
        print(f"Errore: File audio non trovato: {audio_file}")
        sys.exit(1)

    print("Caricamento del modello Whisper (small)... attendere prego.")
    model = whisper.load_model("small")

    print(f"Elaborazione ID: {target_id}")
    ...
    try:
        ...
        final_words = align_words(target_section["text"]["it"], ai_words)
        target_section["words_it"] = final_words
        print(f"  -> Successo! Allineate {len(final_words)} parole.")
        
        with open(data_path, "w", encoding="utf-8") as f:
            json.dump(mass_data, f, ensure_ascii=False, indent=2)
            
        print("Dati salvati correttamente in data.json!")
